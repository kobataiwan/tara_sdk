...
static s32 ae_proc_cmd_drv_ratio(ISP_MODULE *pdrv, INT32 argc, CHAR **argv)
{
	AE_ID id;
	s32 drv_ratio;
	...
	id = simple_strtoul(argv[2], NULL, 0);
	drv_ratio = simple_strtoul(argv[3], NULL, 0);

	dciris_drv_ratio[id] = drv_ratio;
	ae_cmd_printf("set ae_info(%d) drv_ratio(%d) \n", id, dciris_drv_ratio[id]);
	return 0;
}
...
static AE_PROC_CMD ae_proc_w_cmd_list[] = {
	...
	{ "drv_ratio",          ae_proc_cmd_drv_ratio,        "set drv_ratio, param: [id] [drv_ratio]"},

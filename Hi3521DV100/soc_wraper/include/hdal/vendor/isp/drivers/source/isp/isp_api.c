...
#define ISP_IF_SIE_REG_EVENT        ISP_EVENT_SIE_VD | ISP_EVENT_SIE_CRPEND | ISP_EVENT_PARAM_RST
...
static BOOL sie_to_sen_id_map_en;
static UINT32 sie_id_table[ISP_ID_MAX_NUM] = {ISP_ID_1, ISP_ID_2, ISP_ID_3, ISP_ID_4, ISP_ID_5};
static UINT32 sie_to_sen_id_table[ISP_ID_MAX_NUM] = {ISP_ID_1, ISP_ID_2, ISP_ID_3, ISP_ID_4, ISP_ID_5};
static UINT32 ipp_id_table[ISP_ID_MAX_NUM] = {ISP_ID_1, ISP_ID_2, ISP_ID_3, ISP_ID_4, ISP_ID_5};
static UINT32 enc_id_table[ISP_ID_MAX_NUM] = {ISP_ID_1, ISP_ID_2, ISP_ID_3, ISP_ID_4, ISP_ID_5};
...
void isp_api_unreg_if(void)
{
	...
	for (i = 0; i < ISP_ID_MAX_NUM; i++) {
		...
		if (pdev_info->histo_rslt_cap[i] != NULL) {
			isp_uti_mem_free(pdev_info->histo_rslt_cap[i]);
			pdev_info->histo_rslt_cap[i] = NULL;
		}

		if (pdev_info->ipe_subout_buf_cap[i] != NULL) {
			isp_uti_mem_free(pdev_info->ipe_subout_buf_cap[i]);
...
static INT32 isp_api_cb_sie(ISP_ID id, ISP_EVENT evt, UINT32 frame_cnt, void *param)
{
	...
	static ISP_FUNC_EN fun_en[CTL_SIE_ID_MAX_NUM] = {0};
	ISP_FUNC_EN fun_en_tmp = 0;
	static CTL_SIE_ID src_id[CTL_SIE_ID_MAX_NUM] = {0};
	CTL_SIE_ID src_id_tmp = 0;
	...
	sie_to_sen_id_map_en = sie_to_sen_id_map_enable;
	...
	if (sie_ret == CTL_SIE_E_OK) {
		...
		PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "get CTL_SIE_ISP_ITEM_FUNC_EN fail (%d) (0x%X) \r\n", sie_ret, fun_en_tmp);
	}

	sie_ret = ctl_sie_isp_get(id, CTL_SIE_ISP_ITEM_DUPL_SRC_ID, &src_id_tmp);
	if ((sie_ret == CTL_SIE_E_OK) && (src_id_tmp < CTL_SIE_ID_MAX_NUM)) {
		src_id[sie_id] = src_id_tmp;
	} else {
		PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "get CTL_SIE_ISP_ITEM_DUPL_SRC_ID fail (%d) (%d) \r\n", sie_ret, src_id_tmp);
	}

	if (fun_en[sie_id] & ISP_FUNC_EN_SHDR) {
		isp_api_get_frame_num(src_id[sie_id], &frame_num);
		if (frame_num == 1) {
	...
	if ((evt & ISP_EVENT_PARAM_RST) || (evt & ISP_EVENT_SIE_VD)) {
		...
		if (evt & ISP_EVENT_PARAM_RST) {
			...
			isp_src_id_mask[src_id[sie_id]] = 0;
		...
		isp_src_id_mask[src_id[sie_id]] |= 1 << sie_id;
		...
		if ((pdev_info->isp_ae != NULL) && (fun_en[sie_id] & ISP_FUNC_EN_AE) && ((start_3a_frame_cnt[sie_id] >= start_3a_frame_cnt_num) || (evt & ISP_EVENT_PARAM_RST))) {
			if (fun_en[sie_id] & ISP_FUNC_EN_SHDR) {
				...
				ae_src_id_mask[src_id[sie_id]] |= 1 << sie_id;
				if (ae_src_id_cnt[src_id[sie_id]] == frame_num) {
					PRINT_ISP(dbg_mode & ISP_DBG_SIE_CB, "sie vd,    id = %8X, frm = %d, src id = %d, cnt = %d, mask = %d \r\n", id, frame_cnt, src_id[sie_id], ae_src_id_cnt[src_id[sie_id]], ae_src_id_mask[src_id[sie_id]]);
					ae_trig_obj.func_en = fun_en[sie_id];
					ae_trig_obj.src_id_mask = ae_src_id_mask[src_id[sie_id]];
					...
					ae_src_id_mask[src_id[sie_id]] = 0;
				}
			} else {
		...
		if ((pdev_info->isp_awb != NULL) && (fun_en[sie_id] & ISP_FUNC_EN_AWB) && ((start_3a_frame_cnt[sie_id] >= start_3a_frame_cnt_num) || (evt & ISP_EVENT_PARAM_RST))) {
			...
			if (fun_en[sie_id] & ISP_FUNC_EN_SHDR) {
				...
				awb_src_id_mask[src_id[sie_id]] |= 1 << sie_id;
				if (awb_src_id_cnt[src_id[sie_id]] == frame_num) {
					awb_trig_obj.func_en = fun_en[sie_id];
					awb_trig_obj.src_id_mask = awb_src_id_mask[src_id[sie_id]];
					...
					awb_src_id_mask[src_id[sie_id]] = 0;
				}
			} else {
				awb_trig_obj.func_en = fun_en[sie_id];
		...
		if (pdev_info->isp_iq != NULL) {
			...
			if (fun_en[sie_id] & ISP_FUNC_EN_SHDR) {
				iq_src_id_mask[src_id[sie_id]] |= 1 << sie_id;
				iq_trig_obj.sie.src_id_mask = iq_src_id_mask[src_id[sie_id]];
			} else {
	...
	if (evt & ISP_EVENT_SIE_CRPEND) {
		PRINT_ISP(dbg_mode & ISP_DBG_SIE_CB, "sie cr,    id = %8X, frm = %d \r\n", id, frame_cnt);
		if (fun_en[sie_id] & ISP_FUNC_EN_AE) {
			sie_ret = ctl_sie_isp_set(id, CTL_SIE_ISP_ITEM_LA_BUF, pdev_info->la_rslt[sie_id]);
			if (sie_ret != CTL_SIE_E_OK) {
				PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "get CTL_SIE_ISP_ITEM_LA_BUF fail (%d) \r\n", sie_ret);
			}
			ctl_sie_md_relt.md_th_rslt = pdev_info->md_rslt[sie_id]->th;
			sie_ret = ctl_sie_isp_get(id, CTL_SIE_ISP_ITEM_MD_RSLT, &ctl_sie_md_relt);
			if (sie_ret == CTL_SIE_E_OK) {
				...
				PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "set CTL_SIE_ISP_ITEM_MD_RSLT fail (%d) \r\n", sie_ret);
			}
		}

		if (fun_en[sie_id] & ISP_FUNC_EN_AWB) {
			sie_ret = ctl_sie_isp_set(id, CTL_SIE_ISP_ITEM_CA_BUF, pdev_info->ca_rslt[sie_id]);
			if (sie_ret != CTL_SIE_E_OK) {
				PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "set CTL_SIE_ISP_ITEM_CA_BUF fail (%d) \r\n", sie_ret);
			}
		}
	}
...
static INT32 isp_api_cb_ipp(ISP_ID id, ISP_EVENT evt, UINT32 frame_cnt, void *param)
{
	...
	if ((ipp_info != ipp_id) && (evt == ISP_EVENT_IPP_CFGEND_IMM)) {
		pdev_info->is_capture[ipp_id] = TRUE;
		pdev_info->cap_id_mapping_table[ipp_id] = ipp_info;

		if (pdev_info->histo_rslt_cap[ipp_info] == NULL) {
			pdev_info->histo_rslt_cap[ipp_info] = isp_uti_mem_alloc(sizeof(ISP_HISTO_RSLT));
		}

		if (pdev_info->ipe_subout_buf_cap[ipp_info] == NULL) {
			pdev_info->ipe_subout_buf_cap[ipp_info] = isp_uti_mem_alloc(sizeof(ISP_IPE_SUBOUT_BUF));
		}
	} else {
		pdev_info->is_capture[ipp_id] = FALSE;
	}

	pdev_info->ipp_to_sen_id_map_en = ipp_to_sen_id_map_enable;
	pdev_info->ipp_to_sen_id_table[ipp_id] = sen_id;
	pdev_info->ipp_to_sie_id_map_en = ipp_to_sie_id_map_enable;
	...
	if (evt != ISP_EVENT_PARAM_RST) {
		...
		if (rt != E_OK) {
			PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "get CTL_IPP_ISP_ITEM_STRIPE_INFO fail (%d) (%d) (%d) (%d) \r\n", id, pdev_info->is_capture[ipp_id], frame_cnt, rt);
		}
	}

	if (pdev_info->is_capture[ipp_id]) {
		rt = ctl_ipp_isp_get(id, CTL_IPP_ISP_ITEM_EDGE_STCS, &pdev_info->edge_stcs_cap[ipp_info]);
	} else if (evt != ISP_EVENT_PARAM_RST) {
		rt = ctl_ipp_isp_get(id, CTL_IPP_ISP_ITEM_EDGE_STCS, &pdev_info->edge_stcs[ipp_id]);
	} else {
		rt = E_OK;
	}
	if (rt != E_OK) {
		PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "get CTL_IPP_ISP_ITEM_EDGE_STCS fail (%d) (%d) (%d) (%d) \r\n", id, pdev_info->is_capture[ipp_id], frame_cnt, rt);
		return 0;
	}
	...
	case ISP_EVENT_IPP_CFGEND_IMM:
		...
		if (fun_en & ISP_FUNC_EN_AF) {
			if (evt != ISP_EVENT_PARAM_RST) {
				rt = ctl_ipp_isp_get(id, CTL_IPP_ISP_ITEM_VA_RST, pdev_info->va_rslt[ipp_id]);
				rt |= ctl_ipp_isp_get(id, CTL_IPP_ISP_ITEM_VA_INDEP_RST, pdev_info->va_indep_rslt[ipp_id]);
				if (rt != E_OK) {
					PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "get CTL_IPP_ISP_ITEM_VA_RST fail (%d) (%d) (%d) (%d) \r\n", id, pdev_info->is_capture[ipp_id], frame_cnt, rt);
				}
			}
		}
		...
		if (fun_en & ISP_FUNC_EN_WDR) {
			if (pdev_info->is_capture[ipp_id]) {
				rt = ctl_ipp_isp_get(id, CTL_IPP_ISP_ITEM_DCE_HIST_RST, pdev_info->histo_rslt_cap[ipp_info]);
			} else if (evt != ISP_EVENT_PARAM_RST) {
				rt = ctl_ipp_isp_get(id, CTL_IPP_ISP_ITEM_DCE_HIST_RST, pdev_info->histo_rslt[ipp_id]);
			} else {
				rt = E_OK;
			}
			if (rt != E_OK) {
				PRINT_ISP_WRN(dbg_mode & ISP_DBG_WRN_MSG, "get CTL_IPP_ISP_ITEM_DCE_HIST_RST fail (%d) (%d) (%d) (%d) \r\n", id, pdev_info->is_capture[ipp_id], frame_cnt, rt);
			}
		}
			...
			if (evt != ISP_EVENT_PARAM_RST) {
				...
				if (ipp_isp_defog_subout.addr != 0) {
					for (i = 0; i < ISP_SUBOUT_MAX_SIZE / 2; i++) {
						if (pdev_info->is_capture[ipp_id]) {
							pdev_info->ipe_subout_buf_cap[ipp_id]->avg[2 * i] = *((UINT32 *)ipp_isp_defog_subout.addr + i) >> 16 & 0x3FF;
							pdev_info->ipe_subout_buf_cap[ipp_id]->avg[2 * i + 1] = *((UINT32 *)ipp_isp_defog_subout.addr + i) & 0x3FF;
						} else {
...
static INT32 isp_api_cb_enc(ISP_ID id, ISP_EVENT evt, UINT32 frame_cnt, void *param)
{
	...
	pdev_info->enc_to_sen_id_map_en = enc_to_sen_id_map_enable;
	pdev_info->enc_to_sen_id_table[enc_id] = sen_id;
	pdev_info->enc_to_sie_id_map_en = enc_to_sie_id_map_enable;
...
static ER isp_api_get_sensor_data_fmt(ISP_ID id,  BOOL is_from_sie, UINT32 *data_fmt)
{
	...
	if (is_from_sie && sie_to_sen_id_map_en) {
		sen_id = sie_to_sen_id_table[id];
	} else if (!is_from_sie && pdev_info->ipp_to_sen_id_map_en) {
...
ER isp_api_get_sensor_info(ISP_SENSOR_INFO *sensor_info)
{
	...
	for (id = 0; id < ISP_ID_MAX_NUM; id++) {
		if (pdev_info->ipp_to_sen_id_map_en) {
			sen_id = pdev_info->ipp_to_sen_id_table[id];
		} else {
			sen_id = id;
		}
				...
				if (rt == E_OK) {
					if (fun_en & ISP_FUNC_EN_SHDR) {
						if (nvt_get_chip_id() == CHIP_NA51084) {
							sensor_info->src_id_mask[id] = 0x9; // path 1/4
						} else {
							sensor_info->src_id_mask[id] = 0x3; // path 1/2
						}
					} else {
						sensor_info->src_id_mask[id] = 0x1;
					}
				} else {
...
ER isp_api_get_raw(ISP_ID id, ISP_RAW_INFO *raw_info)
{
	...
	if (raw_lock[id] == FALSE) {
		...
		if (rt == E_OK) {
			...
			rt2 = ctl_ipp_isp_get(ipp_id_table[id], CTL_IPP_ISP_ITEM_STATUS_INFO, &ipp_isp_status_info);
			if ((rt2 == E_OK) && (ipp_isp_status_info.flow == CTL_IPP_ISP_FLOW_DIRECT_RAW)) {
...
ER isp_api_get_ir_info(ISP_ID id, ISP_RGBIR_INFO *ir_info)
{
	...
	// NOTE: Does not support ipp->sie mapping
	rt |= ctl_sie_isp_get(sie_id_table[id], CTL_SIE_ISP_ITEM_KDRV_PARAM, &sie_isp_kdrv_param);
	if (rt == E_OK) {
...
void isp_api_set_iq_param(ISP_ID id, ISP_IQ_ITEM isp_iq_item, void *param)
{
	...
	case ISP_IQ_ITEM_SIE_ROI:
		if (!(isp_dbg_get_bypass_eng(isp_id) & ISP_BYPASS_SIE_ROI)) {
			ctl_sie_isp_set(sie_id_table[isp_id], CTL_SIE_ISP_ITEM_STA_ROI_RATIO, (CTL_SIE_ISP_ROI_RATIO *)param);
		}
	break;

	case ISP_IQ_ITEM_SIE_PARAM:
		if (!(isp_dbg_get_bypass_eng(isp_id) & ISP_BYPASS_SIE_PARAM)) {
			ctl_sie_isp_set(sie_id_table[isp_id], CTL_SIE_ISP_ITEM_IQ_PARAM, (CTL_SIE_IQ_PARAM *)param);
	...
	case ISP_IQ_ITEM_DCE_PARAM:
		if (!(isp_dbg_get_bypass_eng(isp_id) & ISP_BYPASS_DCE_PARAM)) {
			ctl_ipp_isp_set(ipp_id_table[isp_id], CTL_IPP_ISP_ITEM_DCE_IQ_PARAM, (CTL_DCE_ISP_IQ_ALL *)param);
		}
	break;
	...
	case ISP_IQ_ITEM_DCE_DC_CENT:
		if (!(isp_dbg_get_bypass_eng(isp_id) & ISP_BYPASS_DCE_DC_CENT)) {
			ctl_ipp_isp_set(ipp_id_table[isp_id], CTL_IPP_ISP_ITEM_DCE_DC_CENT, (CTL_IPP_ISP_DCE_DC_CENT_RATIO *)param);
		}
	break;

	case ISP_IQ_ITEM_IFE2_FILT_TIME:
		if (!(isp_dbg_get_bypass_eng(isp_id) & ISP_BYPASS_IFE2_FILT_TIME)) {
			ctl_ipp_isp_set(ipp_id_table[isp_id], CTL_IPP_ISP_ITEM_IFE2_FILT_TIME, (CTL_IPP_ISP_IFE2_FILTER_TIME *)param);
		}
	break;

	case ISP_IQ_ITEM_IME_LCA_SIZE:
		if (!(isp_dbg_get_bypass_eng(isp_id) & ISP_BYPASS_IME_LCA_SIZE)) {
			ctl_ipp_isp_set(ipp_id_table[isp_id], CTL_IPP_ISP_ITEM_IME_LCA_SIZE, (CTL_IPP_ISP_IME_LCA_SIZE_RATIO *)param);
		}
	break;

	case ISP_IQ_ITEM_IPE_VA_WIN_SIZE:
		if (!(isp_dbg_get_bypass_eng(isp_id) & ISP_BYPASS_IPE_VA_WIN_SIZE)) {
			ctl_ipp_isp_set(ipp_id_table[isp_id], CTL_IPP_ISP_ITEM_VA_WIN_SIZE, (CTL_IPP_ISP_VA_WIN_SIZE_RATIO *)param);
		}
	break;

	case ISP_IQ_ITEM_ENC_3DNR_PARAM:
		if (!(isp_dbg_get_bypass_eng(isp_id) & ISP_BYPASS_ENC_3DNR_PARAM)) {
			kflow_videoenc_set(enc_id_table[isp_id], KFLOW_VIDEOENC_ISP_ITEM_TNR, (KDRV_VDOENC_3DNR *)param);

...
int __NC_VD_COAX_Command_Copy( NC_FORMAT_STANDARD format, NC_VIVO_CH_FORMATDEF vivofmt, unsigned char *Dst, NC_VD_ACP_CMDDEF_STR *pCMD )
{
	...
	else if( (format == FMT_AHD20) || (format == FMT_AHD30) )
	{
		...
		else if( vivofmt == AHD30_8M_12_5P || vivofmt == AHD30_8M_15P )
		{
			cmd_cnt = __NC_VD_COAX_Command_Each_Copy( Dst, pCMD->ahd_4_5m );
		}
		else
	...
	{
#ifdef TVI_FMT_COAX_DEBUG
		cmd_cnt = __NC_VD_COAX_Command_Each_Copy( Dst, pCMD->tvi_v1_0 );
#else
		if( (vivofmt == TVI_4M_30P) || (vivofmt == TVI_4M_25P) || (vivofmt == TVI_4M_15P) || (vivofmt == TVI_8M_15P) || (vivofmt == TVI_8M_12_5P) )
		{
...
void coax_tx_init( void *p_param )
{
	...
	gpio_i2c_write(raptor3_i2c_addr[coax_tx->vd_dev], 0xA8, 0x08);  // MPP_TST_SEL1
	gpio_i2c_write(raptor3_i2c_addr[coax_tx->vd_dev], 0xA9, 0x09);  // MPP_TST_SEL2
	gpio_i2c_write(raptor3_i2c_addr[coax_tx->vd_dev], 0xAA, 0x0A);  // MPP_TST_SEL3
	gpio_i2c_write(raptor3_i2c_addr[coax_tx->vd_dev], 0xAB, 0x0B);  // MPP_TST_SEL4
...
void coax_tx_cmd_send( void *p_param )
{
	...
	else 
	{
		...
		{
		gpio_i2c_write(raptor3_i2c_addr[coax_tx->vd_dev], 0xFF, 0x03+((ch%4)/2) );
			gpio_i2c_write(raptor3_i2c_addr[coax_tx->vd_dev], tx_shot_addr+((ch%2)*0x80), 0x08);  // 3x09 0x01 -> 0x08
			msleep(150);
			gpio_i2c_write(raptor3_i2c_addr[coax_tx->vd_dev], tx_shot_addr+((ch%2)*0x80), 0x00);
		}
		else
...
#else
				gpio_i2c_write(raptor3_i2c_addr[coax_tx->vd_dev], tx_cmd_addr+((ch%2)*0x80)+i, TCP_CMD_Stop_v20[i]);
#endif
			}
		}
		else
...
void coax_tx_16bit_cmd_send( void *p_param )
{
	...
	NC_VD_COAX_Init_STR *CoaxVal = __NC_VD_COAX_InitFormat_Get(vivofmt);    // Get from Coax_Tx_Init Table
	NC_VD_ACP_CMDDEF_STR *pCMD   = __NC_VD_ACP_Get_CommandFormat_Get(cmd);  // Get From Coax_Tx_Command Table
	printk("[drv_coax]Dev: %d Ch: %d Command >>>>> %s\n",coax_tx->vd_dev, ch, pCMD->name );
...
void coax_rx_buffer_clear(void *p_param)
{
	...
	gpio_i2c_write(raptor3_i2c_addr[coax_val->vd_dev], 0x3A+((ch%2)*0x80), 0x01);
	gpio_i2c_write(raptor3_i2c_addr[coax_val->vd_dev], 0x3A+((ch%2)*0x80), 0x00);
}
...
void coax_fw_ready_header_check_from_isp_recv(void *p_param)
{
	...
	{
		printk(">>>>> DRV[%s:%d] CH:%d, this camera can update, please, wait! = 0x%x\n", __func__, __LINE__, ch, gpio_i2c_read( raptor3_i2c_addr[ch/4], 0x51+((ch%2)*0x80) ) );
		ret = FW_SUCCESS;
	}
	else
	{
...
void coax_fw_end_cmd_to_isp_send(void *p_param)
{
	...
	gpio_i2c_write( raptor3_i2c_addr[devnum], 0x03+((ch%2)*0x80), 0x0D);   // line
	gpio_i2c_write( raptor3_i2c_addr[devnum], 0x05+((ch%2)*0x80), 0x03);   // tx_line_count
	gpio_i2c_write( raptor3_i2c_addr[devnum], 0x0A+((ch%2)*0x80), 0x03);   // tx_line_count_max

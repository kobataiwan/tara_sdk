...
HI_S32 SAMPLE_RGN_CreateLine(RGN_HANDLE Handle, HI_U32  ChnId,HI_U32 u32Num,MOD_ID_E u32ModId)
{
    ...
    for (i=Handle; i<(Handle + u32Num); i++)
    {
        ...
        stChnAttr.unChnAttr.stLineChn.stLinePoints[0].s32X = 100 * (i - Handle);
        stChnAttr.unChnAttr.stLineChn.stLinePoints[0].s32Y= 60 * (i - Handle);
        stChnAttr.unChnAttr.stLineChn.stLinePoints[1].s32X = 100 * (i - Handle);
        stChnAttr.unChnAttr.stLineChn.stLinePoints[1].s32Y= 60* (i - Handle)+300;
...
HI_VOID *SAMPLE_RGN_VpssOsdReverse(void *pData)
{
    ...
    while (HI_FALSE == bExit)
    {
        ...
        s32Ret = HI_MPI_VPSS_GetGrpRegionLuma(pstOsdReverseInfo->VpssGrp, &(pstOsdReverseInfo->stLumaRgnInfo), au32LumaData, 500);
        if (HI_SUCCESS != s32Ret)
        {

...
#endif
#ifdef __DEBUG
#define __D(fmt, args...) fprintf(stderr, "msg: " fmt, ## args)
#else
#define __D(fmt, args...)
#endif
...
/**
* @brief Initialize message queue for wis-streamer(RTSP server).
* Initialize message queue for wis-streamer.
* @param None.
* @return message ID.
*/
int msg_queue_wis_streamer()
{
	key_t key;
	int msqid;
	if ((key = ftok(STREAMER_PATH , 'B')) == -1) {
		perror("ftok");
		return -1;
	}
	msqid = Msg_Init(key);

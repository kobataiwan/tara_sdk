...
#if 0
fprintf(stderr, "seq %d ", fSeqNo);
if (fCurrentTimestamp != fCurrentTimestamp_tmp) {
fCurrentTimestamp_tmp = fCurrentTimestamp;
fprintf(stderr, "timestamp %lu\n", fCurrentTimestamp);
} else
fprintf(stderr, "\n");
#endif
...
//fprintf(stderr, "%s overflow %d\n", __func__, fOutBuf->overflowDataSize());
...
//fprintf(stderr, "%s, read a new frame from source\n", __func__);
...
fprintf(stderr, "MultiFramedRTPSink::afterGettingFrame1(): The input frame data was too large for our buffer size (%d) %d bytes of trailing data was dropped! Current value is %d).", bufferSize, numTruncatedBytes, OutPacketBuffer::maxSize);
  }
...
//fprintf(stderr,"%s try getting another\n", __func__);
      packFrame();
    }
  }
}
